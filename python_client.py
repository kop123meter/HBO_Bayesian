...
def objective( X):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            ...
            #arr = bytes(data2, 'utf-8')
            #client_socket.sendall(arr)
            #print("sent 1")
        ...
        #
        # Extract the reward value (Y) from the result
        received_data=0.2
        Y = float(received_data) * (X_list[0][0]+X_list[0][2])
    

        return Y # this should pass the reward function from java server after calculating the mean quality and average AI inference response time
...
#problem = JavaRewardBayesianOptimization(domain=space,constraints=None)
'''
max_iter  = 4
problem.run_optimization(max_iter = max_iter)                                      
                                         
selectedx=problem.X
#problem._compute_acquisitions(selectedx)
