// ///
// /**
//  * This is old balancer with per AI model trained considering all the bins not the updated ones
//  * the measured W is omitted and instead we rely on estimated one. the problem with this code is that
//  * 1- Wi_estimated does not change much along time because rohT and rohD of each model is trained for all points (even old data)
//  * 2- when Wi_estimated comes from even past/obsolated data, it might happen that the order of AI models in terms of triangles impact is not accurate
//  *
...
//
//public class balancer2 implements Runnable {
//
//    int binCap=5;
...
//
//
//    public balancer2(MainActivity mInstance) {
//
...
//                mInstance.initial_totT=totTris;
//            }
//            if (meanDkk == 0)
//                meanDkk = meanDk;
//            if (pred_meanD == 0)
...
//
//
//                    mInstance.thr_models.get(aiIndx).put(totTris, meanThr); //adds to the end of list correct:  should have real throughput for the regression
...
//
//
//                  //correct:  should have predicted value removes from the head (older data) -> to then add to the tail
...
//                        ai_sensitivity=   Math.sqrt(  Math.pow (((meanThr-initial_thr )/(totTris- iniTris ) ),2)  +
...
//        avg_est_H =  (double) Math.round((double) avg_est_H * 100) / 100;
//        avg_msred_H =  (double) Math.round((double) avg_msred_H * 100) / 100;
//
...
//
//                double msr_thr=  mInstance.thr_models.get(i).get(totTris).get( mInstance.thr_models.get(i).get(totTris).size()-1);// get the last/current value
...
//            double total_est_weights = mInstance.est_weights.stream().mapToDouble(f -> f.doubleValue()).sum();
//
//            double msrd_Weighted_thr = sum_currentWt/total_est_weights;// this is average weighted throughput of current period
//            msrd_Weighted_thr = (double) Math.round((double) msrd_Weighted_thr * 100) / 100; // sum of estimated weights
//
//
//
...
//        sb .append(",").append(taskView.getModels().get(taskView.getCurrentModel()))
//                .append(" ,")  .append(",").append(mInstance.est_weights.get(ai_indx));
//        sb.append('\n');
//
//        try (PrintWriter writer = new PrintWriter(new FileOutputStream(FILEPATH, true))) {
//            writer.write(sb.toString());
