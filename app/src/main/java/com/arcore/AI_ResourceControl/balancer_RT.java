...
// RE based on response time but has some glitch because Rt is very fine-grain
...
public class balancer_RT implements Runnable {
   ...
   // int aiIndx;
    ...
    public balancer_RT(MainActivity mInstance) {
...
// this gets above bincap sooner than the rest of lists, so, we need to keep the size not more than 5
        ...
        for (int aiIndx=0; aiIndx<Ai_count;aiIndx++) {
            ...
            meanThr = mInstance.getResponseT(aiIndx)[0];// after the objects are decimated
            while((1000/meanThr) > 200 || (1000/meanThr) <1) // we wanna get a correct value
                meanThr = mInstance.getResponseT(aiIndx)[0];// after the objects are decimated
...
//                } //Specific AI  throughput model
                ...
                writeThr(meanThr, predThr, trainedThr,aiIndx,100*mape);// It has predicted and real throughput of task AI[index]
            ...
            //    double msr_thr=  mInstance.thr_models.get(i).get(mInstance.total_tris).get( mInstance.thr_models.get(i).get(mInstance.total_tris).size()-1);// get the last/current value
             ...
             double msr_thr= Iterables.getLast( mInstance.thr_models.get(i).get(last_tris), null);// last added thr
               ...
               // double msr_thr = mInstance.thr_models.get(i).get(totTris).get(0);// get the throughput of current tris for each model
                ...
                sum_baseWt += ((est_wi  * mInstance.des_Rt_weight) *mInstance.baseline_AIRt.get(i)  );//sum(Normalized Wi * Hbase)
...
// just weights are based on response time parameters and the rest includeing measured RE and estimated RE should include throughput
     ...
     //   double PRoAI = (sum_currentWt / sum_baseWt) ;// should include throughput
        double PRoAI = (sum_baseWt/sum_currentWt  ) ;// this should be inveresed since we use response T instead of throughput
...
//@ * ******************* calculate average AI throughput from weighted model -> roh'T= sigma(rohT_i * w^i)/N roh'D= sigma(rohD_i * wi)/Ndelta'= sigma(delta_i * wi)/N
        //@ ******************* calculate average AI throughput from weighted model   *********** @//
            ...
            if (mInstance.objectCount == 0) {
                ...
                double re_mape = 0.0;
                if(acc_throughput && reMsrd!= Double.NaN) {// we collect RE data if Average throughput model is accurate
...
//                   r1=0.3f;
    ...
    public void writeThr(double realThr, double predThr, boolean trainedFlag,int aiIndx,double ai_acc){ // AI throughput information for each task individually and response time for all models
