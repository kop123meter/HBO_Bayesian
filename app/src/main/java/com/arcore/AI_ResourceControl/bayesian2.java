...
import static java.lang.Math.min;
 ...
 public class bayesian2 implements Runnable {
...
//    double tris_share[];
  ...
  //  double tMin[] ;
    ...
    public bayesian2(MainActivity mInstance) {
...
///*temp deactive to make sure of heuristic func works
    ...
    void startSceneTimer() { // THIS IS TO APPLY JUST ONE INSTANCE OF DELEGATE AND CALCULATE REWARD AT THE END
        CountDownTimer sceneTimer = new CountDownTimer(21000, 3000) {// 25 TIMES (50000/2000) DATA COLLECTION EVERY 5S ,
            ...
            public void onFinish() {
                ...
                if(mInstance.bys_baseline1_2)// this is for HBO compared to the frist two baselines
                 reward =0.4*mInstance. avgq + (0.6*avgLatency);
    ...
    public void writeRT( ){ // this is to collect the response time of all AIS after change in the AI device
        ...
        try (PrintWriter writer = new PrintWriter(new FileOutputStream(FILEPATH, true))) {
            for (int i=0;i<mInstance.mList.size();i++)
            {
                ...
                avg_AIlatencyPeriod+=(expected_time/actual_rpT);//normalized over curr time this is because we want to have this value minimized

                double cur_latency=expected_time/actual_rpT;
                double tmp_lastLatency=mInstance.last_latencyInstanceN;

                if(i==0 && tmp_lastLatency!=0)// we check atleast one istance to make sure our latency is not noisy
                {
                    if(cur_latency/tmp_lastLatency >1.4)
                        isnoisy=true;
                    else
                        isnoisy=false;
                }
                if(i==0 && isnoisy==false)// update last latency
